...
# Product rule: one active device/session per account. A refresh-token row
# *is* a session here — created at login, revoked at logout (or password
# reset), and naturally stale once past expires_at. "Active" means neither
# of those, so logging out or letting a session expire always frees the
# account up for a new login; nothing here needs its own cleanup job.

def has_active_session(db: Session, user_id: int) -> bool:
    """True if `user_id` already has a refresh-token session that is
...
def raise_if_active_session(db: Session, user_id: int) -> None:
    """Blocks a new login when `user_id` already has an active session
    elsewhere — the enforcement side of the single-session-per-account
    rule. Callers use this right before issuing a fresh token pair for an
    *existing* user logging in (never for first-time account creation,
    which can't have a prior session to conflict with)."""
    if has_active_session(db, user_id):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail=(
                "This account is already logged in on another device. "
                "Log out there, or wait for that session to expire, before "
                "logging in again."
            ),
        )
