...
real app would use) against an isolated SQLite database, exactly
the way app.main would. This exercises the actual endpoint/service/schema
...
# Use a safe test value here.
# Individual tests that depend on this value explicitly override it with
# monkeypatch so the developer's real .env can never make the test flaky.
...
def _mock_msg91_success(identifier="919876543210"):
    return patch(
        ...
        return_value=FakeMSG91Response(
            {
                "type": "success",
                "message": identifier,
            }
...
def test_valid_access_token_calls_msg91_with_authkey_and_never_leaks_it(
    ...
    # IMPORTANT:
    # The application loads .env during imports. On a developer machine
    # that .env may contain a real MSG91 authkey.
    #
    # This test must therefore explicitly override the environment value
    # so that the assertion is deterministic and never depends on a real
    ...
    # The service reads MSG91_AUTHKEY at import time, so also update the
    # module-level value used by the service for this test.
    monkeypatch.setattr(
        msg91_service,
        "MSG91_AUTHKEY",
        "test-msg91-authkey",
    ...
    # Backend must call MSG91 server-to-server with the test authkey.
    ...
    assert (
        call_kwargs["json"]["authkey"]
        == "test-msg91-authkey"
    )

    assert (
        call_kwargs["json"]["access-token"]
        == "good-token"
    )

    # The authkey must never appear anywhere in the response we send back.
...
def test_empty_access_token_rejected_before_calling_msg91():
    ...
    # Pydantic min_length=1 rejects this at the schema level -> 422,
    # and MSG91 is never contacted.
...
# ---------------------------------------------------------------------------
# 3. MSG91 API failure (network error / 5xx) -> 502,
...
def test_creates_user_from_pending_signup_if_registered_first():
    ...
    try:
        pending = models.PendingSignup(
            ...
            date_of_birth=__import__("datetime").date(
                2000,
                1,
                1,
            ),
    ...
    try:
        ...
        assert (
            db.query(models.PendingSignup).count()
            == 0
        )
...
#    (second verification for the same number)
# ---------------------------------------------------------------------------
...
def test_issued_access_token_is_a_valid_jwt_for_the_verified_user():
    ...
    # And that access token actually works
...
def test_email_identifier_from_msg91_is_handled():
    ...
    assert (
        body["user"]["is_email_verified"]
        is True
    )


# ---------------------------------------------------------------------------
# 8. Unparseable MSG91 identifier
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 9. Duplicate-account race and generic DB errors
#
# Requirement:
# "duplicate account" / "database errors" must not surface
# as an unhandled 500.
# ---------------------------------------------------------------------------
...
def test_concurrent_duplicate_identifier_does_not_500():
    ...
    number racing each other.

    The first call's user-creation commit wins.

    The second should hit the IntegrityError path and gracefully log
    the caller into the row the first request just created — never a
    raw 500.
    """
    ...
    real_get_user = (
        auth_routes_module._get_user_by_identifier
    )
    ...
    def racy_get_user(
        ...
        if call_count["n"] == 1:
            # Our own lookup finds nothing yet, so we're about to take
            # the create path.
            #
            # Simulate a concurrent request winning the race right now,
            # before our own commit: insert the competing row directly
            # so our subsequent db.add()+db.commit() below hits the real
        ...
        # Use the real lookup so it finds the row the "other" request
        # created.
...
def test_database_error_during_finalize_returns_503_not_500():
    ...
    assert (
        "database error"
        in r.json()["detail"].lower()
    )

    # No half-created user should be left behind
    # after the rollback.
    db = TestSessionLocal()
